...
class OrderHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            sql = "insert into ih_order_info(oi_user_id,oi_house_id,oi_begin_date,oi_end_date,oi_days,oi_house_price," \
                  "oi_amount) values(%(user_id)s,%(house_id)s,%(begin_date)s,%(end_date)s,%(days)s,%(price)s," \
                  "%(amount)s);update ih_house_info set hi_order_count=hi_order_count+1 where hi_house_id=%(house_id)s;"
